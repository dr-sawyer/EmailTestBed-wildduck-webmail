    ...
    apiClient.mailboxes.list(req.user, true, (err, mailboxes) => {
        ...
        apiClient.messages.get(req.user, mailbox, result.value.message, (err, messageData) => {
            ...
            info.push({
                ...
                value: () => {
                    let compl = ''
                    if (messageData.from.address && messageData.from.address != 'null') {
                        compl = '&nbsp;&lt' + messageData.from.address + '&gt'
                    }
                    return tools.getAddressesHTML(messageData.from, true) + compl
                }
            });
            ...
            if (messageData.to) {
                info.push({
                    key: 'To',
                    isHtml: true,
                    value: tools.getAddressesHTML(messageData.to, true)
                });
            }
